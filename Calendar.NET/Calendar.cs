...
namespace Calendar.NET
{
    ...
    public class Calendar : UserControl
    {
        ...
        public Dictionary<DateTime,Rectangle> dateRectangles = new Dictionary<DateTime, Rectangle>();
        ...
        public event EventHandler DateCalenderClick;
        public event EventHandler CalenderDateChange;
        ...
        public DateTime CalendarDate
        {
            ...
            {
                _calendarDate = value;
                CalenderDateChange?.Invoke(this,null);
                Refresh();
            }
        }
        ...
        private void CalendarMouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                foreach (var rectangle in dateRectangles)
                {
                    if (rectangle.Value.Contains(e.X, e.Y))
                    {
                        DateCalenderClick?.Invoke(rectangle.Key, e);
                        return;
                    }
                }
            }
        ...
        private void BtnTodayButtonClicked(object sender)
        {
            ...
            Refresh();
            // Todo : click event
            TodayButtonClick?.Invoke(sender,null);
        }
        ...
        private void BtnLeftButtonClicked(object sender)
        {
            ...
            Refresh();
            // Todo : click event
            LeftButtonClick?.Invoke(sender, null);
        }
        ...
        private void BtnRightButtonClicked(object sender)
        {
            ...
            Refresh();
            // Todo : click event
            RightButtonClick?.Invoke(sender, null);
        }
            ...
            for (int y = 0; y < numWeeks; y++)
            {
                for (int x = 0; x < 7; x++)
                {
                         ...
                         && counter == DateTime.Now.Day)
                            {
                                ...
                                // 오늘 1일
                                dateRectangles.Add(new DateTime(_calendarDate.Year, _calendarDate.Month, counter), new Rectangle(xStart, yStart, cellWidth, cellHeight));
                                //g.DrawString(counter.ToString(CultureInfo.InvariantCulture), _dayOfWeekFont, Brushes.Green, xStart, yStart);
                                if (CalendarDate == new DateTime(_calendarDate.Year, _calendarDate.Month, counter))
                                {
                                    g.DrawRectangle(new Pen(Color.Black, 2.0f), new Rectangle(xStart + 2, yStart + 2, cellWidth - 3, cellHeight - 3));
                                }
                            }
                            else
                            {
                         ...
                         && counter == DateTime.Now.Day)
                            {
                                ...
                                // 오늘
                                dateRectangles.Add(new DateTime(_calendarDate.Year, _calendarDate.Month, counter), new Rectangle(xStart, yStart, cellWidth, cellHeight));
                                //g.DrawString(counter.ToString(CultureInfo.InvariantCulture), _dayOfWeekFont, Brushes.Blue, xStart, yStart);
                            ...
                            else
                            {
                                ...
                                // 나머지
                                dateRectangles.Add(new DateTime(_calendarDate.Year, _calendarDate.Month, counter), new Rectangle(xStart, yStart, cellWidth, cellHeight));
                                //g.DrawString(counter.ToString(CultureInfo.InvariantCulture), _dayOfWeekFont, Brushes.Red, xStart, yStart);
                    ...
                    else if (rogueDays > 0)
                    {
                        ...
                        rogueDays--;
                        // 저번달 
                        dateRectangles.Add(new DateTime(_calendarDate.Year, _calendarDate.Month,1).AddMonths(-1).AddDays(dm-1), new Rectangle(xStart, yStart, cellWidth, cellHeight));

                        if (CalendarDate == new DateTime(_calendarDate.Year, _calendarDate.Month, 1).AddMonths(-1).AddDays(dm - 1))
                        {
                            g.DrawRectangle(new Pen(Color.Black, 2.0f), new Rectangle(xStart, yStart, cellWidth, cellHeight));
                        }
                        else
                        {
                            g.DrawRectangle(Pens.DarkGray, xStart, yStart, cellWidth, cellHeight);
                        ...
                        //g.DrawString(dm.ToString(CultureInfo.InvariantCulture), _dayOfWeekFont, Brushes.Red, xStart, yStart);
                    }
                    ...
                    if (rogueDays == 0 && counter > DateTime.DaysInMonth(_calendarDate.Year, _calendarDate.Month))
                    {
                        ...
                        else
                        {
                            if (counter2 == 1)
                            {
                               ...
                               // g.DrawString(_calendarDate.AddMonths(1).ToString("MMM") + " " + counter2.ToString(CultureInfo.InvariantCulture), _dayOfWeekFont, Brushes.LightSalmon, xStart, yStart);
                                // 다음달 1일
                                dateRectangles.Add(new DateTime(_calendarDate.Year, _calendarDate.Month, 1).AddMonths(1).AddDays(counter2 - 1), new Rectangle(xStart, yStart, cellWidth, cellHeight));
                            ...
                            else
                            {
                                ...
                                //g.DrawString(counter2.ToString(CultureInfo.InvariantCulture), _dayOfWeekFont, Brushes.LightSalmon, xStart, yStart);
                                // 다음달 2일부터
                                dateRectangles.Add(new DateTime(_calendarDate.Year, _calendarDate.Month, 1).AddMonths(1).AddDays(counter2 - 1), new Rectangle(xStart, yStart, cellWidth, cellHeight));
                            }
                            counter2++;
                ...
                foreach (IEvent v in _events)
                {
                    ...
                    if (NeedsRendering(v, dt))
                    {
                        ...
                        if (!v.Enabled && _showDashedBorderOnDisabledEvents)
                        {
                            ...
                            g.DrawString("1", _dayOfWeekFont,Brushes.LightSalmon, point.X + 1, point.Y + offsetY);
                        }
